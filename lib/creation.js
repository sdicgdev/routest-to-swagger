...
export function add_path(paths, route, ...remaining){
  if(!route) return paths
  const swagger_url = swaggerize_url(route.get('path'));
  const swagger_path_definition = swaggerize_definition(route);
  const updated_paths = paths.setIn([swagger_url, swagger_path_definition.get('method')], swagger_path_definition.get('definition'))
  return add_path(updated_paths, ...remaining)
}

function swaggerize_url(route){
  const hard_route = route.match(/^\//)?route:`/${route}`; 
  const swagger_url = parameter_list(hard_route).reduce((path, regex, n) => path.replace(regex, `{var_${n}}`) , hard_route)
  return swagger_url.replace(/:/g, '');
}

function swaggerize_definition(route, proto=PATH_PROTOTYPE){
  const new_path = proto.set('parameters', parameters_for(route))
  return Map({
